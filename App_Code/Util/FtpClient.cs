...
public class FtpClient
{
	...
	public void Login()
	{
		...
		catch(Exception ex)
		{
			...
			throw new FtpException("Não foi possível conectar ao servidor de FTP.",ex);
		}
	...
	public void RenameFile(string oldFileName,string newFileName, bool overwrite)
	{
		...
		if ( !overwrite && this.GetFileList(newFileName).Length > 0 ) throw new FtpException("O arquivo já existe.");
	...
	public void MakeDir(string dirName)
	{
        ...
        if (this.resultCode != 250 && this.resultCode != 257 && this.resultCode != 550) throw new FtpException(this.result.Substring(4));
	...
	private Socket createDataSocket()
	{
		...
		for (int i = 0; i < len && partCount <= 6; i++)
		{
			...
			if ( char.IsDigit(ch) )
                ...
                throw new FtpException("Malformed PASV result: " + result); //"Malformed PASV result: "
